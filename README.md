...
I'm a Principal Software Engineer specializing in distributed systems, user interfaces, accessibility, and security.

> I have over 11 years of experience in designing, building, and shipping tools that are used daily for systems-critical tasks by folks at: Google, Apple, Microsoft, Intel, IBM, Cisco, AWS, Red Hat, Oracle, Github, Gitlab, Bitbucket, StackExchange, Pixar, Autodesk, Sony, EA, Nike, Twitch, and even the Large Hardon Collider at CERN- just to name a few.
...
let technologies = {
  lanugages: {
    expertAt: [
      'JavaScript (ES5) => ES14 // plus Node.js, Deno, Bun.js, TypeScript, JSDoc, etc',
...
#### Heckadecimal (Consulting)

Working with tech companies to solve specific problems related to full-stack development, massively distributed systems, and user interfaces with a focus on usability, accessibility, privacy, and security. I help define the work to be done, break it down into accomplishable subtasks, prioritize everything into a sensible ordering to maximize team efficiency, communicate upcoming work with stakeholders and team members, occasionally also assigning tickets to other engineers and myself, and help knock out tickets in priority order, as well as ensuring everything is thoroughly tested and working properly, well documented, and as easy as possible to maintain for other engineers.

Recently, I worked with a YC-backed ai startup to plan, implement, test, and ship new features and bug fixes in a react app and python backend that enables users to generate custom data sets and fine-tune LLM's using said data sets.

#### Kaiju Keyboards

Created a website to promote the company, make blog posts about our progress, and list custom keyboards for sale (using sveltekit and Node.js). I also created firmware using mostly C (and some Rust) for ESP32/ESP8266 microcontrollers, integrating with QMK/VIA to not only handle input from keys, but also fully integrated trackballs that control the cursor on screen with no configuration required out of the box, and to handle other input mechanisms as well, like rotary encoders and faders/sliders.
...
Contributed primarily to astro UI, a react app (with typescript) for managing all of your big data jobs using apache airflow, and contributed to the backend services and API's in GO and Node.js that power it. Within my first month I brought test coverage up from 0% to over 50%. In addition to regular feature work and bug fixes, I also onboarded and mentored several other engineers, and consistently drove progress towards better patterns around accessibility, security, and code readability/developer experience.
